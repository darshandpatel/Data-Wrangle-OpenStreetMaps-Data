...
def process_file(filename, fields):
    ...
    alphanumerix_expr = re.compile(r'[^a-zA-Z0-9]')
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            for field in process_fields:
                if field == 'name':
                    if alphanumerix_expr.match(line[field]) or line[field] == "NULL":
                        dictionary[fields[field]] = line['rdf-schema#label'].strip()
                    else:
                        dictionary[fields[field]] = line[field].strip()
                elif field == 'synonym':
                    if line[field] != 'NULL':
                        dictionary[fields[field]] = parse_array(line[field])
                    else:
                        dictionary[fields[field]] = None
                elif field == 'rdf-schema#label':
                    if parenthesis_expr.match(line[field]):
                        dictionary[fields[field]] = re.sub(r'\(.*\)','',line[field]).strip()
                    else:
                        dictionary[fields[field]] = line[field].strip()
                elif field == 'URI' or field == 'rdf-schema#comment':
                    dictionary[fields[field]] = line[field].strip()
                else:
                    if 'classification' not in dictionary:
                        dictionary['classification'] = {}
                        if line[field] == "NULL":
                            dictionary['classification'][fields[field]] = None
                        else:
                            dictionary['classification'][fields[field]] = line[field].strip()
                    else:
                        if line[field] == "NULL":
                            dictionary['classification'][fields[field]] = None
                        else:
